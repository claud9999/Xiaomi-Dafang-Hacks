...
if [ -n "$F_cmd" ]; then
  case "$F_cmd" in
  get_services)
    services="auto-night-detection debug-on-osd ftp_server mqtt-control mqtt-status mqtt-broker mqtt-detection mqtt-snap onvif-srvd recording rtsp sound-on-startup telegram-bot timelapse"
